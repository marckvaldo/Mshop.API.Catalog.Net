...
namespace Mshop.Application.UseCases.Cache.Products.ListProductCache
{
    public class ListProductCache : BaseUseCase, IListProductCache
    {
        ...
        {
            var produts = await _productCacheRepository.FilterPaginated(request, cancellationToken);

            if (produts is null)
            {
                _buildCache.Handle();
                produts = await _productRepository.FilterPaginated(request, cancellationToken);
            }
